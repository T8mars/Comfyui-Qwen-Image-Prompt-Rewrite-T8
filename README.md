...
默认 `after_run` 在任务完成或报错后退出自有 `llama-server`。`keep_loaded` 只缓存当前 profile；模型、视觉组件或上下文配置变化时先关闭旧进程。服务仅绑定 `127.0.0.1`。图片按插口顺序放在同一次多模态请求中，视觉副本等比例缩至最多 1,048,576 像素；原图尺寸留在结果里供画布使用。模型必须返回官方要求的严格 JSON 字段，否则节点报错，不会凭空补齐或静默忽略图片。
