...
- **PE Rewrite T8**：输入文字、任务类型、画幅比例、输出语言、透明图开关、T2I 与编辑模型、视觉组件、卸载策略、种子，以及 `image_1` 至 `image_10`。比例可选 `auto`、`1:1`、`1:2`、`2:3`、`3:4`、`4:5`、`16:9`、`9:16`、`21:9`、`9:21`、`5:4`、`4:3`、`2:1`；语言可选 `auto`、`中文`、`English`。`auto` 任务在无图时调用 T2I，有图时调用 I2I。每个端口只接一张图，不接批次；输出 `None` 的端口会跳过，其余图片按端口顺序重新编号。输出改写文字、结构化 `PE_RESULT` 和诊断信息。
...
图片引用规则：只按实际有图的端口顺序编号，`None` 端口跳过，结果的 `image_input_ports` 记录每个编号对应的原端口。无图文生图不使用图片标签；单图编辑可以自然描述，也允许保留 `<image1>`，无引号的 `<image>` 会规范为 `<image1>`，不再改成“原图”。多图编辑必须使用全部 `<image1>` 至 `<imageN>`，无编号的 `<image>` 不能推测指向哪张图，仍要求模型重写；引号内要求精确呈现的画面文字保持原样。
