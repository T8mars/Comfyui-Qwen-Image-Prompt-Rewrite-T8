...
- **PE Rewrite T8**：输入文字、任务类型、画幅比例、输出语言、透明图开关、T2I 与编辑模型、视觉组件、卸载策略、种子，以及按顺序连接的 `image_1` 至 `image_10`。比例可选 `auto`、`1:1`、`1:2`、`2:3`、`3:4`、`4:5`、`16:9`、`9:16`、`21:9`、`9:21`、`5:4`、`4:3`、`2:1`；语言可选 `auto`、`中文`、`English`。`auto` 任务在无图时调用 T2I，有图时调用 I2I。必须从 `image_1` 连续连接，每个端口只接一张图，不接批次。输出改写文字、结构化 `PE_RESULT` 和诊断信息。
...
改写生成上限按官方任务配置设为 T2I 16,256 tokens、编辑 24,000 tokens；相应的 llama.cpp 上下文为 T2I 24,576、1–5 图编辑 49,152、6–10 图编辑 65,536 tokens。更大上下文会增加运行时显存占用。若模型仍触及生成或上下文上限，节点会关闭思考并重试一次，重试结果仍须通过完整的 JSON、图片引用、语言和画幅校验；再次截断时错误信息会显示请求上限和服务端返回的 token 用量。

图片引用规则：连接但输出 `None` 的图片端口不计入实际输入图数；实际图片仍须从 `image_1` 开始连续排列。无图文生图和单图编辑的改写正文均不使用 `<image>` 或 `<image1>` 标签；多图编辑必须按输入顺序使用 `<image1>` 至 `<imageN>`。单图编辑中模型偶发输出的无引号 `<image>` / `<image1>` 会改成“原图”或“the source image”，引号内画面文字保持原样；多图或无图时的无编号占位符仍要求模型重写，不会把不存在的参考图交给下游出图节点。
