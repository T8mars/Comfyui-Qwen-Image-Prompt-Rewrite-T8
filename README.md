...
图片引用规则：无图文生图和单图编辑的改写正文均不使用 `<image>` 或 `<image1>` 标签；多图编辑必须按输入顺序使用 `<image1>` 至 `<imageN>`。若模型输出了无编号的 `<image>` 占位符，节点会按当前图数明确要求模型重写，并在重试时直接索取最终 JSON；仍不合规则报错，不会把不存在的参考图交给下游出图节点。
